#!/bin/bash
# resolve-file.sh — three-layer override chain for prompt/agent files
# usage: resolve-file.sh <relative-path> [data-dir]
#
# checks in order:
#   1. .claude/cc-queue/<path>    (project override)
#   2. <data-dir>/<path>          (user override; falls back to $CLAUDE_PLUGIN_DATA)
#   3. <skill-root>/references/<path>   (bundled default)
#
# outputs the resolved file's content to stdout.
#
# mirrors planning's resolve-file.sh contract — same prefix scheme, different
# project override prefix (`cc-queue` instead of `exec-plan`).
...
if [ -f ".claude/cc-queue/$path" ]; then
    cat ".claude/cc-queue/$path"
