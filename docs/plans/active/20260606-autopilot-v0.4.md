...
> Cloudflare Worker (see `20260606-loom-relay-hub.md`), which owns the webhook and exposes
...
- Dependencies:
  - **Hard dependency on `20260606-loom-relay-hub.md`** (the MCP hub) for Feature 2 at runtime.
...
- [ ] `references/relay-control.md`: how to point autopilot at a deployed `loom-relay` (MCP server entry in `.mcp.json` with the CF Access service-token headers `CF-Access-Client-Id`/`CF-Access-Client-Secret` via env, supported commands, latency caveat); cross-link `20260606-loom-relay-hub.md`
...
**Dependency (separate plan):** `loom-relay` (`20260606-loom-relay-hub.md`) must be implemented
