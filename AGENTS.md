# AGENTS.md — loom marketplace

Personal Codex plugin marketplace. Three own plugins (`autopilot`, `kit`,
...
- **Own plugins only.** Edit `plugins/autopilot/`, `plugins/kit/`, and `plugins/slicer/`.
  ...
  `.Codex-plugin/marketplace.json` and must stay references (no copies, no drift).
...
- **Versioning.** Bump `version` in the plugin's `.Codex-plugin/plugin.json` and add
...
- **No manual marketplace.json drift.** The manifest is the source of truth for what
  ...
  entries — do not hand-edit Codex's internal `~/.Codex/plugins/*.json` state.
